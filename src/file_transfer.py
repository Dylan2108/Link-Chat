...
class FileTransferManager:
    def __init__(self, ui, network_manager):
        self.ui = ui
    ...
    def send_file(self, dest_mac, file_path):
        ...
        if not self.network.send_message(dest_mac, MessageType.File_Request, file_info):
            print("Error enviando solicitud de archivo.")
            return False

        print(f"Enviando '{file_name}' ({file_size} bytes)...")
        ...
        with open(file_path, "rb") as f:
            ...
            while True:
                ...
                if not self.network.send_message(
                    dest_mac, MessageType.File_Chunk, chunk_header
                ):
