...
class NetworkManager:
    ...
    def start(self, interface):
        ...
        if not self.socket_manager.create_socket(interface):
            return False
        self.running = True
    ...
    def receive_loop(self):
        # Bucle de recepcion
        while self.running:
            frame = self.socket_manager.receive_frame()
            if frame:
    ...
    def send_message(self, dest_mac, message_type, data):
        ...
        frame = self.frame_builder.build_ethernet_frame(
            dest_mac, self.socket_manager.mac_address, Protocol.ETH_TYPE, payload
        )
